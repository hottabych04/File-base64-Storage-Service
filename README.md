...
- Все основные компоненты сервиса расположены в директории `src/main/java/com/hottabych04/app`.
  ...
  * Для пеобразования `Page<File>` в `PageFileDto` реализован класс `PageFileReadMapper`. 
...
а для `save` метода обработкой ошибок, возникших в результате отправки невалидного тела запроса, занимается `FileExceptionHandler` в пакете `http.handlers`:
```json
{
